...
const CreatePost = () => {
  ...
  const [imgPreview, setImgPreview] = useState(null); // for display
  const [imgBase64, setImgBase64] = useState(null);   // for backend upload
  const imgRef = useRef(null);

  const { data: authUser } = useQueryClient().getQueryData(["authUser"])
  ...
  const { mutate: createPost, isError, isPending } = useMutation({
    mutationFn: async ({ text, img }) => {
      const res = await fetch("/api/v1/posts/create", {
        ...
        body: JSON.stringify({ text, img }), // img is Base64 here
      });
  ...
  const handleImgChange = (e) => {
    ...
    reader.onloadend = () => {
      setImgBase64(reader.result); // Base64 string
    };
  };
